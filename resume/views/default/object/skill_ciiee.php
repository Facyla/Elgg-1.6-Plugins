...
if (!$compact) {
  ...
  <div>
    <p>
      <?php
      echo elgg_view('resume/importancebar', array('importance' => $vars['entity']->importance));
      if ($vars['entity']->skilltype) { echo '<p>' . elgg_echo('resume:skill_ciiee:typology') . '&nbsp;: ' . elgg_echo('resume:referential:c2i2e:' . $vars['entity']->skilltype) . '</p>'; }
      ?>
    </p>
    ...
    echo '<p>';
      ...
      if (($page_owner->guid == get_loggedin_user()->guid) && (get_context() != "profileprint")) {
        echo '<a href="' . $vars['url'] . 'mod/resume/skill_ciiee.php?id=' . $vars['entity']->getGUID() . '">' . elgg_echo('resume:edit') . '</a>&nbsp; ';
  ...
  if ($page_owner->guid == get_loggedin_user()->guid) {
    echo '<a href="' . $vars['url'] . 'mod/resume/skill_ciiee.php?id=' . $vars['entity']->getGUID() . '" title="' . elgg_echo('edit') . '">' . elgg_echo('resume:referential:c2i2e:' . $vars['entity']->skilltype) . '</a> &nbsp; ' 
    ...
    echo '<a href="' . $vars['entity']->getURL() . '">' . elgg_echo('resume:referential:c2i2e:' . $vars['entity']->skilltype) . '</a><br />';
  }
}
