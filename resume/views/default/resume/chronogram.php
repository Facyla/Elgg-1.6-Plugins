...
if ($vars['full']) $fullview = true; else $fullview = false;
...
$height = 500; // Global height of figure
$padding = 10; // Inner margin (content display in it)
$transp = 0.4; // Initial transparency
$gridtransp = 0.2; // Grid transparency
$txttransp = 0.8; // Initial text transparency
...
function protovis_experience_addupperbar($entity, $config, $color) {
  ...
  $end = $entity->enddate;
  if ($entity->ongoing == 'ongoing') $end = time();
...
foreach ($educations as $ent) {
  ...
  // Pour un listing @todo chronologique
  //$dat1 .= date('m/Y',$ent->startdate) . ' - ' . date('m/Y',$ent->enddate) . ' : <a href="' . $ent->getURL() . '">' . $ent->heading . '</a> (' . $ent->importance . '%) - ' . $ent->title . '<br />';
}
foreach ($workexperiences as $ent) {
  $js_lower .= protovis_experience_addupperbar($ent, $PROTOVIS_EXPERIENCE, "rgba(0,200,200,$transp)");
  // Pour un listing @todo chronologique
...
$area1 .= elgg_echo('resume:chronogram:description') . '<br /><br />';

/* Légende */
if ($fullview) {
  $legend = '<div style="float:right; margin:0 0 6px 0; padding:6px; width:220px;">
      <h4>Légende</h4>
      <br />
      ...
      <b>Notes :</b>
      <ul>
        <li><em>Des informations complémentaires s\'affichent au survol d\'un élément (laissez la souris immobile quelques instants).</em></li>
        <li><em>Chacun des "blocs" est cliquable et vous renvoie sur la page détaillée de l\'élément mis en valeur.</em></li>
      </ul>
    </div>';
} else {
  $legend = '<div style="float:left; width:45%; margin:0 6px 2px 0; padding:6px;">
      <h4>Légende</h4>
      <br />
      <div class="protovis_experience_legend" style="background:rgb(200,200,0);"></div>Etudes et formations
      <div class="clearfloat"></div>
    ...
    <div style="float:left; width:50%; margin:0; padding:6px;">
      <b>Notes :</b>
      <ul>
        <li><em>Des informations complémentaires s\'affichent au survol d\'un élément (laissez la souris immobile quelques instants).</em></li>
        <li><em>Chacun des "blocs" est cliquable et vous renvoie sur la page détaillée de l\'élément mis en valeur.</em></li>
...
if ($fullview) { $area1 .= $legend; } // Si pas sur le côté, cf. après la visualisation
...
vis.add(pv.Rule)
  ...
  .strokeStyle(pv.color('rgba(0, 0, 0, $gridtransp)'));
...
// Si pas sur le côté, légende après la visualisation
if (!$fullview) $area1 .= '<div class="clearfloat"></div>' . $legend;

$area1 .= '<div class="clearfloat"></div>';


echo elgg_view_title($title) . '<div class="contentWrapper" style="padding:5px 10px; min-height:500px;">' . $area1 . '</div>';
