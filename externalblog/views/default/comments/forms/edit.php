...
if ( isset($vars['entity']) ) {
    ...
    */
    ...
    if (!isloggedin() && $opencomments->comments_formedit_disabled !='yes' && get_context() != 'pages') {
        ...
        $form_body_anonymous .= '<p>'.elgg_echo("email"). '  *<br />'.elgg_view('input/text', array(
                'internalname' 	=> 'comment_anonymous_mail',
                'value' 	=> '',
                'class'         => 'required email'
                )).'</p>';
/*
        $form_body_anonymous .= '<p>'.$nb1 . '+'. $nb2 . ' = ? *<br />' .elgg_view('input/text', array(
                'internalname' 	=> 'equ',
                'value' 	=> '',
...
*/                
		// THE QUESTION
        if (!empty($opencomments->la_question_title)) {       
			$extended = '';
			 if (!empty($opencomments->la_question_help)) {
				 $extended = ' title="'.$opencomments->la_question_help.'" ';
			 }
			
			$form_body_anonymous .= '<p>'.$opencomments->la_question_title .'*<br />' .elgg_view('input/text', array(
					'internalname' 	=> 'la_reponse',
					'value' 	=> '',
					'class'         => 'required  tooltips-w',
					'extended'	=> $extended,
					)). '</p>';
