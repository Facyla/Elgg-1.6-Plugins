...
if (!elgg_view_exists('file/upload')) {
  ...
  <form id="mediaUpload" action="<?php echo $vars['url']; ?>action/file/upload" method="post" enctype="multipart/form-data">
    <p><label for="upload">
      <?php echo elgg_echo("file:file") . '<br />' . elgg_view("input/file",array('internalname' => 'upload', 'js' => 'id="upload"')); ?>
    </label></p>
    <p><label>
      <?php echo elgg_echo("title") . '<br />' . elgg_view("input/text", array("internalname" => "title", "value" => $title, )); ?>
    </label></p>
    <p><label for="filedescription"><?php echo elgg_echo("description"); ?><br />
      <textarea class="input-textarea" name="description" id="filedescription"></textarea>
    </label></p>
    <p><label>
      <?php echo elgg_echo("tags") . '<br />' . elgg_view("input/tags", array("internalname" => "tags", "value" => $tags, )); ?>
    </label></p>
    <p><label>
      <?php echo elgg_echo('access') . '<br />' . elgg_view('input/access', array('internalname' => 'access_id','value' => $access_id)); ?>
    </label></p>
