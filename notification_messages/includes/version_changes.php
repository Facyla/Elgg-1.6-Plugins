...
## or in a more user-friendly format ("clean") 
## with an extra (optional) parameter
function PHPDiff($old, $new, $render = "diff") {
   # split the source text into arrays of lines
   $t1 = explode("\n",$old);
   $x=array_pop($t1); 
   if (($render == "clean") || ($render == "html")) {
   } else {
  ...
  if ($render == "clean") {
    ...
    foreach($actions as $act) {
      ...
      if ($op>0) {
        ...
        if ($op==1) $out[] = "\nSuppression entre les lignes $xstr à $y1 :"; // Ligne supprimée ("d") entre les lignes $xstr et $y1
        elseif ($op==3) $out[] = "\nModifications entre les lignes $xstr à $ystr :"; // Ligne modifiée ("c") entre les lignes $xstr et $ystr
        while ($x0<$x1) { $out[] = '-   '.$t1[$x0]; $x0++; }   # deleted elems
        if ($op==2) $out[] = "\nInsertion entre les lignes $x1 à $ystr :"; // Nouvelle ligne ("a") entre les lignes $x1 et $ystr
        elseif ($op==3) $out[] = '--- remplacé par ---';
    ...
    /* END User-friendly rendering */
  } else if ($render == "html") {
    /* START User-friendly rendering in HTML */
    foreach($actions as $act) {
      if ($act==1) { $op|=$act; $x1++; continue; }
      if ($act==2) { $op|=$act; $y1++; continue; }
      if ($op>0) {
        $xstr = ($x1==($x0+1)) ? $x1 : ($x0+1).",$x1";
        $ystr = ($y1==($y0+1)) ? $y1 : ($y0+1).",$y1";
        if ($op==1) $out[] = "<u>Suppression entre les lignes $xstr à $y1 :</u><br />"; // Ligne supprimée ("d") entre les lignes $xstr et $y1
        elseif ($op==3) $out[] = "<u>Modifications entre les lignes $xstr à $ystr :</u><br />"; // Ligne modifiée ("c") entre les lignes $xstr et $ystr
        while ($x0<$x1) { $out[] = '-   '.strip_tags($t1[$x0]).'<br />'; $x0++; }   # deleted elems
        if ($op==2) $out[] = "<u>Insertion entre les lignes $x1 à $ystr :</u><br />"; // Nouvelle ligne ("a") entre les lignes $x1 et $ystr
        elseif ($op==3) $out[] = '--- remplacé par ---<br />';
        while ($y0<$y1) { $out[] = '+   '.strip_tags($t2[$y0]).'<br />'; $y0++; }   # added elems
      }
      $x1++; $x0=$x1;
      $y1++; $y0=$y1;
      $op=0;
    }
    /* END User-friendly rendering in HTML */

  } else {
    /* START Unix diff(1) rendering */
    foreach($actions as $act) {
      if ($act==1) { $op|=$act; $x1++; continue; }
      if ($act==2) { $op|=$act; $y1++; continue; }
      if ($op>0) {
